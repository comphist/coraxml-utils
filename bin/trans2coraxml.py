...
def importTextFromDocx(infile):
    ...
    for sec in doc.paragraphs:
        for tok in range(len(sec.runs)):
            ...
            if run.font.italic is True and normal is False and italic is False:
            ...
            elif run.font.italic is not True and normal is False and italic is not False:
            ...
            if run.font.highlight_color is not None and begin is False:
                begin = True
            ...
            elif run.font.highlight_color is None and begin is True:
                begin = False
        ...
        # If the ending of the heading is in this paragraph, "@H" is added.
            ...
            if italic_end == 0:
                ...
                if match:
                    text_all.append(
                        name + site + str(line) + "\t" + sec.text[match.end(0):]
                    )
                else:
        ...
        # If the beginning of the question is in this paragraph, "+Q" is added.
            ...
            if match:
                text_all.append(
                    ...
                    + str(line)
                    + "\t"
                    + sec.text[match.end(0): marked_begin]
                    + "+Q "
                    + sec.text[marked_begin:]
                )
            else:
                text_all.append(
                    name
            ...
            if marked_end == 0:
                ...
                if match:
                    text_all.append(
                        ...
                        + str(line)
                        + "\t"
                        + sec.text[match.end(0): marked_end]
                        + " @Q"
                        + sec.text[marked_end:]
                    )
                else:
...
if __name__ == "__main__":
    ...
    parser.add_argument("infile", help="Eingabedatei (Transkription)")
    ...
    if doc:
        ...
        if args.postprocessing:
            postprocessor = getattr(
                coraxml_utils.modifier, args.postprocessing + "_postprocess"
            )
