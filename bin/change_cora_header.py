...
parser.add_argument("folder", help="A folder containing CoraXML files.")
parser.add_argument(
    "-a",
    "--add",
    help="A tab or csv file containing additional information to be added to the CoraXML header.",
)
parser.add_argument(
    "-d",
    "--delete",
    action="store_true",
    default=False,
    help="Delete unwanted information from CoraXML header.",
)
...
if args.add:
    if not os.path.isfile(args.add):
        ...
        for index, line in enumerate(open(args.add, mode="r", encoding="utf-8")):
            linelist = [col.strip() for col in line.split("\t")]
            if index == 0:
                for col in linelist:
                    additional_data_keys.append(col)
            else:
                additional_data[linelist[0]] = dict()
                for col in range(1, len(additional_data_keys)):
                    additional_data[linelist[0]][additional_data_keys[col]] = linelist[col]
    ...
    for filename in os.listdir(args.folder):
        ...
        try:
                    ...
                    if sigle in additional_data:
                        for additional_info in additional_data_keys[1:]:
                            additional_text += (
                                additional_info
                                + ": "
                                + additional_data[sigle][additional_info]
                                + "\n"
                            )
                    else:
            ...
            if new_header_text:
                ...
                with open(args.folder + "/" + filename, mode="w", encoding="utf-8") as xmlfile:
